...
Java.perform(function() {
    ...
    "frida-agent-32.so" ,"frida-agent-arm.so",   "frida-helper-32",  "linjector-2"  ,"linjector-4",
    "frida-agent-64.so",  "frida-agent-arm64.so",  "linjector-1",     "linjector-3"];
    ...
    exec5.implementation = function(cmd, env, dir) {
        ...
        if (cmd == "which") {
            var fakeCmd = "justafakecommandthatcannotexistsusingthisshouldthowanexceptionwheneversuiscalled";
            console.log("Bypass which command");
            send({type: 'root'})
            return exec1.call(this, fakeCmd);
        }
    ...
    Interceptor.attach(Module.findExportByName(null, "fstatat64"), {
        onEnter: function(args) {
            var path1 = Memory.readCString(args[1]);
            var path = path1.split("/");
            var executable = path[path.length - 1];
            var shouldFakeReturn = (RootBinaries.indexOf(executable) > -1)
            if (shouldFakeReturn) {
                Memory.writeUtf8String(args[1], "/ggezxxx");
                console.log("Bypass native fopen >> "+path1);
                send({type: 'root'})
            }
            send({
